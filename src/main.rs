...
use ws::{
    CloseCode, Error, ErrorKind, Handler, Handshake, Result, Sender, WebSocket
};
...
fn main() {
    let server = WebSocket::new(|out| Server { out: out, ping_timeout: None }).unwrap();

    let broadcaster = server.broadcaster();
    
    let periodic = thread::spawn(move || loop {
        broadcaster.send("It's Yann's web app!").unwrap();
